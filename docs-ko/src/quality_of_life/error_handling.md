...
Par는 명시적은 시도(Try) 값으로써 오류를 표현하고, 이를 활용해 프로세스 간에 오류를 전파하는 가벼운 문법인 `try`/`catch`/`throw`를 제공한다. 마지막으로 오류 전파 시에 범위 안에 자원이 남아 있을 때는 [자동 정리](../types/auto_cleanup.md)로써 올바르게 닫히거나 롤백되도록 한다.
