...
박스는 공유 가능하지만, 감싸져 있는 타입이 데이터라도 그 자체로는 `data`를 만족하지 않는다.

```par
box [Int] Int  // share이지만 data는 아님
box Int        // 역시 share이지만 data는 아님
```

박스는 유예된 연산을 담고 있으므로, 데이터 연산을 적용하려면 [`.unbox`](./box.md#소멸)를 사용해 인스턴스화한 뒤 값을 얻어야 한다. 박스는 아래에서 살펴볼 `number`나 `signed`도 만족하지 않는다.
